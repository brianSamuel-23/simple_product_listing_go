...
type ProductPriceRepository interface {
	GetProductPrices(filter bson.M, page uint16, limit uint8, sortField string, sortOrder string) ([]model.ProductPrice, uint16, error)
}

func NewProductPriceRepository(client *mongo.Client) ProductPriceRepository {
	db := client.Database("example_db")
	collection := db.Collection("product_price")
	return &productPriceRepository{collection}
}

func (r *productPriceRepository) GetProductPrices(filter bson.M, page uint16, limit uint8, sortField string, sortOrder string) ([]model.ProductPrice, uint16, error) {
	...
	var result []dto.ProductPriceAggregateResult
